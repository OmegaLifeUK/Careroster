...
export default function WeekCalendar({ weekDays = [], shifts = [], carers = [], clients = [], onShiftClick, onShiftUpdate, isLoading }) {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
        {Array.isArray(weekDays) && weekDays.map((day, idx) => (
  ...
  if (!Array.isArray(weekDays) || weekDays.length === 0) {
    return <div className="text-center py-8 text-gray-500">No dates to display</div>;
  }
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <div className={`w-2 h-2 rounded-full ${statusColors[shift.status] || statusColors.scheduled}`} />
