...
export default function ClockInOut({ shift = null, carer = null, client = null, timeAttendance = null, entityType = 'shift' }) {
  ...
  const createLocationMismatchNotification = async (locationMatch, isClockIn) => {
    if (locationMatch === "mismatch" || locationMatch === "not_available") {
      try {
        ...
        // Determine notification entity based on entity type
        const NotificationEntity = entityType === 'shift' 
          ? base44.entities.Notification 
          : base44.entities.DomCareNotification;

        const notificationPromises = admins.map(admin =>
          NotificationEntity.create({
            recipient_id: admin.email,
            title: `Location Alert: ${carer.full_name}`,
            message: `${carer.full_name} ${isClockIn ? 'clocked in' : 'clocked out'} outside the expected location for ${client.full_name}'s ${entityType}.`,
            type: "clock_alert",
            priority: "high",
            is_read: false,
    ...
    mutationFn: async () => {
      ...
      try {
        ...
        if (entityType === 'shift') {
          ...
          await base44.entities.Visit.update(shift.id, { status: "in_progress" });
        }
  ...
  const clockOutMutation = useMutation({
    mutationFn: async ({ location, notes, completedTasks }) => {
      setLoading(true);
      setLocationError(null);
      ...
      try {
        ...
        if (entityType === 'shift') {
          ...
          await base44.entities.Visit.update(shift.id, {
            status: "completed",
            actual_start: timeAttendance.clock_in_time,
            actual_end: clockOutTime.toISOString(),
            completion_notes: notes,
          });
        }
  ...
  const handleClockIn = () => {
    if (confirm(`Ready to start your ${entityType}?`)) {
        ...
        <Card className="p-3 bg-blue-50 border-blue-200">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-blue-900">{entityType === 'shift' ? 'Shift' : 'Visit'} Completed</span>
            </div>
      ...
      {timeAttendance?.clock_in_time && !timeAttendance.clock_out_time && (
        <div className="space-y-4">
          ...
          <div className="p-4 bg-white border rounded-lg">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-semibold flex items-center gap-2">
                ...
                {entityType === 'shift' ? 'Shift' : 'Visit'} Notes
              </h4>
              <Button
                type="button"
                variant="outline"
            ...
            <Textarea
              ...
              placeholder={`Describe the ${entityType}... (AI can help you write this!)`}
              rows={6}
          ...
          >
            ...
            {loadingLocation ? "Getting Location..." : `Clock Out & Complete ${entityType === 'shift' ? 'Shift' : 'Visit'}`}
          </Button>
        </div>
      )}
